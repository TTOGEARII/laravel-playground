...
class PersonaResolver
{
    ...
    public function systemPrompt(AgentSession $session, ?string $game = null): string
    {
        ...
        return implode("\n", [
            ...
            '- 캐릭터를 조회할 때는 **사용자가 말한 이름/별명을 그대로** search_characters 의 keyword 로 넣어 먼저 조회한다. '
                .'DB 가 커뮤니티 애칭도 정식 등록명으로 보유하는 경우가 많으니, "이건 OO의 별명이겠지" 하고 임의로 다른 캐릭터 이름으로 바꿔 조회하지 않는다. '
                .'그 이름으로 못 찾을 때만 대체 표기를 시도하고, 끝내 없으면 "찾지 못했다"고 솔직히 말한다.',
            '- **한 캐릭터를 조회할 때 search_characters 를 여러 keyword 로 반복 호출하지 않는다**(한 번의 결과로 답한다). '
                .'데이터가 적은 게임(특히 브라운더스트2 는 코스튬별로 행이 나뉘고 티어·빌드·조합 정보가 없을 수 있다)은 '
                .'조회된 속성·성급·코스튬 종류만으로 간결히 답하고, 없는 정보를 지어내지 않는다. 조회 결과가 있으면 반드시 그걸로 답을 마무리한다.',
            '- 특정 캐릭터의 "조합/팀 구성"을 물으면 먼저 search_characters 로 그 캐릭터를 조회한다. '
                .'호요버스(원신·스타레일·젠존제)·명조는 DB 에 구조화된 팀 멤버 목록이 없고 조합이 **영상**으로만 있으니, '
                .'반환된 추천 조합 영상과 그 캐릭터의 역할(격파/딜러/서포터 등)·속성을 근거로 답한다. '
                .'**질문한 캐릭터가 반드시 포함된 조합만** 말하고, 툴 결과에 없는 팀원 이름을 임의로 지어내지 않는다. '
                .'구체 편성이 필요하면 "아래 조합 영상 참고"로 안내한다.',
            '- search_characters 가 돌려준 추천 조합 영상은 카드로 표시된다. 본문에는 영상 **제목으로만** 언급하고 '
                .'유튜브 URL 을 새로 지어내 붙이지 않는다(가짜 링크 방지). 실제 검색 링크가 필요하면 search_youtube_videos 를 쓴다.',
            ...$context,
            '',
            '## 이름·명칭 (해외 게임 오역 방지)',
