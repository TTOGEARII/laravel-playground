...
@section('content')
    <section class="shell">
        <div class="phero">
            <a class="back" href="{{ route('my-wife-bot.characters') }}">← 목록으로</a>
            <span class="tag">🤖 MY WIFE BOT</span>
            <h1>{{ $character ? '캐릭터 수정' : '챗봇 추가하기' }}</h1>
            <p>페르소나를 촘촘히 채울수록 캐릭터가 더 일관되게 대화해요.</p>
        </div>
    </section>

    <section class="shell" style="padding-bottom:var(--s6)">
        <form action="{{ $character ? route('my-wife-bot.characters.update', $character) : route('my-wife-bot.characters.store') }}" method="POST" enctype="multipart/form-data" class="card" style="max-width:720px;gap:var(--s4)">
            @csrf
            @if($character)
                @method('PUT')
            @endif
            ...
            <div class="form-ai-box">
                ...
                <textarea id="novel_source" rows="5" class="input" style="resize:vertical;min-height:120px" placeholder="예) 『늑대와 향신료』 — 행상인 크래프트 로렌스는 어느 날 수레에서 풍요의 신 '현랑 호로'를 만난다. 호로는 늑대 귀와 꼬리를 가진 소녀의 모습으로..."></textarea>
            ...
            <div class="field">
                ...
                <input type="text" id="character_name" name="character_name" value="{{ old('character_name', $character?->name) }}" maxlength="30" required class="input">
            ...
            <div class="field">
                ...
                <input type="text" id="short_intro" name="short_intro" value="{{ old('short_intro', $character?->short_intro) }}" maxlength="50" required class="input">
                @error('short_intro')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="field">
                ...
                <textarea id="character_detail" name="character_detail" maxlength="1000" rows="6" class="input" style="resize:vertical;min-height:140px">{{ old('character_detail', $character?->character_detail) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="personality" name="personality" maxlength="500" rows="3" class="input" style="resize:vertical;min-height:96px" placeholder="예: 도도하지만 정이 많다. 호기심이 강하고 장난기가 있다.">{{ old('personality', $character?->personality) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="appearance" name="appearance" maxlength="500" rows="3" class="input" style="resize:vertical;min-height:96px" placeholder="예: 은발에 붉은 눈동자, 늑대 귀와 꼬리를 가진 소녀.">{{ old('appearance', $character?->appearance) }}</textarea>
            ...
            <div class="grid-2">
                <div class="field">
                    ...
                    <input type="text" id="likes" name="likes" value="{{ old('likes', $character?->likes) }}" maxlength="255" class="input" placeholder="예: 사과, 술, 칭찬받는 것">
                    @error('likes')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="field">
                    <label for="dislikes">싫어하는 것 <span class="form-optional">(선택)</span></label>
                    <input type="text" id="dislikes" name="dislikes" value="{{ old('dislikes', $character?->dislikes) }}" maxlength="255" class="input" placeholder="예: 외로움, 거짓말">
            ...
            <div class="field">
                ...
                <input type="text" id="user_alias" name="user_alias" value="{{ old('user_alias', $character?->user_alias) }}" maxlength="50" class="input" placeholder="예: 그대, 주인님, 자기야">
            ...
            <div class="field">
                ...
                <textarea id="example_dialogue" name="example_dialogue" maxlength="2000" rows="5" class="input" style="resize:vertical;min-height:120px" placeholder="유저: 너 이름이 뭐야?&#10;캐릭터: 후후, 나는 현랑 호로다. 현명한 늑대지.&#10;&#10;유저: 사과 좋아해?&#10;캐릭터: 당연하지! 사과는 내가 제일 좋아하는 거란다.">{{ old('example_dialogue', $character?->example_dialogue) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="world_setting" name="world_setting" maxlength="2000" rows="5" class="input" style="resize:vertical;min-height:120px" placeholder="예: 중세풍 행상인들이 오가는 대륙. 정령과 신이 잊혀가는 시대. 풍요의 신 호로는 수백 년간 마을의 밀 농사를 지켜왔다...">{{ old('world_setting', $character?->world_setting) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="relationships" name="relationships" maxlength="2000" rows="4" class="input" style="resize:vertical;min-height:110px" placeholder="예:&#10;로렌스 — 여행을 함께하는 행상인 동료. '멍청한 양치기'라고 놀리며 부른다.&#10;노라 — 경쟁 관계의 양치기. 서먹하지만 인정하고 있다.">{{ old('relationships', $character?->relationships) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="user_persona" name="user_persona" maxlength="1000" rows="3" class="input" style="resize:vertical;min-height:96px" placeholder="예: 유저는 이 마을에 처음 온 젊은 행상인으로, 캐릭터와는 오늘 처음 만난 사이다.">{{ old('user_persona', $character?->user_persona) }}</textarea>
            ...
            <div class="field">
                ...
                <textarea id="speech_style" name="speech_style" rows="3" class="input" style="resize:vertical;min-height:96px" placeholder="예: 반말을 쓰고, ~요로 끝낸다. 친근한 말투.">{{ old('speech_style', $character?->speech_style) }}</textarea>
                @error('speech_style')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="field">
                ...
                <div class="form-intro-row">
                    <textarea id="intro_message" name="intro_message" maxlength="1000" rows="3" class="input" style="resize:vertical;min-height:96px" placeholder="대화 시작 시 캐릭터가 보낼 첫 메시지. 비워두면 저장 시 AI가 자동 생성합니다.">{{ old('intro_message', $character?->intro_message) }}</textarea>
                    @if($character)
            ...
            <div class="row" style="gap:var(--s2);margin-top:var(--s2)">
                <button type="submit" class="btn btn-sm">{{ $character ? '수정하기' : '캐릭터 생성' }}</button>
                @if($character)
                    <a href="{{ route('my-wife-bot.characters') }}" class="btn btn-soft">취소</a>
                @endif
            </div>
        </form>
    </section>
